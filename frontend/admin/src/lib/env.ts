...
const parseEnv = () => {
  if (isServer) {
    ...
    if (!result.success) {
      console.error("❌ Invalid admin server environment variables:", JSON.stringify(result.error.format(), null, 2));
    ...
    if (!result.success) {
      console.error("❌ Invalid admin client environment variables:", JSON.stringify(result.error.format(), null, 2));
